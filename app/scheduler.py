...
def parse_queue_times_from_announcement(text: str) -> List[Dict[str, Any]]:
    ...
    # ============= ПАТТЕРН 3.5: "розпочнеться раніше" без номера черги (у тому самому реченні) =============
    # "Підчерга 6.1 - відключення триватиме до 04:00; відключення розпочнеться раніше - о 05:00"
    # Шукаємо контекст: номер черги в межах 200 символів назад
    pattern3_5 = r'(?:від|ви|зне)?(?:ключення|мкнення|струмлення)\s+(?:розпочнеться|почнеться)\s+раніше\s+[-–—]\s+о\s+(\d{1,2}):(\d{2})'
    
    for match in re.finditer(pattern3_5, text, re.IGNORECASE):
        pos_key = (match.start(), match.end())
        if pos_key in processed_positions:
            continue
        
        # Шукаємо номер черги в контексті (200 символів назад)
        context_start = max(0, match.start() - 200)
        context = text[context_start:match.start()]
        
        # Шукаємо останню згадку черги в контексті
        queue_match = None
        for queue_pattern in re.finditer(r'(?:підчерг[ауиі]?|черг[ауиі]?)\s+(\d+\.\d+)', context, re.IGNORECASE):
            queue_match = queue_pattern
        ...
        if queue_match:
            ...
            results.append({
                ...
                'is_power_off': True,
                'action_type': 'earlier_start_no_dash',
                'matched_text': f"[Черга {queue} з контексту] {match.group(0)}"
            })
            processed_positions.add(pos_key)
            logger.info(f"📋 [РАНІШЕ-КОНТЕКСТ] Черга {queue}: початок о {hour}:00")
